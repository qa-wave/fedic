...
import { useVisualStyle, visualStyles } from "@/components/shared/style-switcher";
import { CheckCircle2, Palette, User, Save } from "lucide-react";
import { toast } from "sonner";
import { motion } from "framer-motion";

export default function PortalProfilPage() {
  const { style, setStyle } = useVisualStyle();
  const [name, setName] = useState("Tomáš Mertin");
  const [email, setEmail] = useState("tomas.mertin@gmail.com");
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Vaše kontaktní informace</CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Vyberte si vzhled portálu. Změna se projeví okamžitě.</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
                ...
                >
                  ...
                  <div className="bg-card p-3">
                    <div className="flex items-center justify-between">
                      ...
                      {isActive && <Badge className="text-[10px]">Aktivní</Badge>}
                    </div>
                  </div>
                </motion.button>
              );
